...
class WordModel(nn.Module):
    def __init__(self, args, is_testing=False):
        ...
        self.wordEmbeddings = nn.Embedding(self.word_vocab_size, self.word_dim , padding_idx=0)
        ...
        # out_vocab_size is default 5K
        # The most frequent 5K words in the training corpus (it is also a way to regularize parameters)
        self.hidden2word = nn.Linear(self.rnn_size, self.out_vocab_size)
        # Initialize hidden state with zeros
    ...
    def forward(self, batch):
        word_embeds = self.wordEmbeddings(batch)
...
class AddBiLSTMModel(nn.Module):
    def __init__(self, args, is_testing=False):
        ...
        ####### Inf2W
        # binary addition vectors for derivational part are coming with dimension subword_vocab_size
        self.comp_lstm = nn.LSTM(subword_vocab_size, \
                               self.rnn_size, \
                               num_layers=1,\
                               bidirectional=True, \
                               batch_first=True,\
    ...
    def forward(self, batch):
        #### I2W
        batch = batch.view(self.batch_size*self.num_steps, self.bilstm_num_steps, -1)
        _, self.comp_hidden = self.comp_lstm(batch, self.comp_hidden)
        h_n_f = self.comp_hidden[0][0]
        h_n_b = self.comp_hidden[0][1]
        exp_bias = self.we_bias.unsqueeze(0)
        exp_bias = exp_bias.expand(self.batch_size*self.num_steps, self.word_dim)
